...
export default function SignupPage() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-[#10141a] px-4 py-12">
            {/* Background glow */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                <div className="absolute -top-40 -right-40 w-[600px] h-[600px] rounded-full bg-indigo-500/10 blur-[120px]" />
                <div className="absolute -bottom-40 -left-40 w-[600px] h-[600px] rounded-full bg-brand-500/10 blur-[120px]" />
            </div>
            ...
            >
                ...
                <div className="glass-card p-8">
                    <div className="mb-6">
                        ...
                        <p className="text-slate-400 text-sm mt-1">Start planning smarter trips with AI</p>
                    </div>
                    ...
                    <AnimatePresence>
                        {success && (
                            <motion.div
                                ...
                                className="mb-5 flex items-center gap-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 px-4 py-3 text-sm text-emerald-400"
                            >
                    ...
                    <form onSubmit={handleSubmit} noValidate className="space-y-4">
                        {/* Name */}
                        <div>
                            <label htmlFor="signup-name" className="block text-xs font-medium text-slate-300 mb-1.5">
                            ...
                            />
                            {fieldErrors.name?.map((m) => <p key={m} className="mt-1 text-xs text-red-400">{m}</p>)}
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500/50 transition-all"
                            />
                            {fieldErrors.email?.map((m) => <p key={m} className="mt-1 text-xs text-red-400">{m}</p>)}
                        </div>
                        ...
                        <div>
                            ...
                            {fieldErrors.password?.map((m) => <p key={m} className="mt-1 text-xs text-red-400">{m}</p>)}
                        </div>

                        {/* Confirm password */}
                        <div>
                            <label htmlFor="signup-confirm" className="block text-xs font-medium text-slate-300 mb-1.5">
                                Confirm password
                            </label>
                            <div className="relative">
                                <input
                            ...
                            {fieldErrors.confirmPassword?.map((m) => <p key={m} className="mt-1 text-xs text-red-400">{m}</p>)}
                        </div>
