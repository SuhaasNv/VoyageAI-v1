    ...
    return runWithRequestContext(req, async () => {
        ...
        try {
            ...
            if (cached) {
                ...
                return NextResponse.json({ success: true, data: formatAIResponse(cached as object, {
                    confidence: computeConfidence({ mode: "LLM_GROUNDED" }),
                    reasoning:  `Destination overview for "${name}" served from cache (LLM + image API).`,
                    sources:    ["LLM knowledge base", "Destination image API"],
                }) });
            }
...
Schema:
{
  ...
  "bestTimeToVisit": "A short string indicating the best season/months and why."
            ...
            return NextResponse.json({ success: true, data: formatAIResponse(result, {
                confidence: computeConfidence({ mode: imageUrl ? "LLM_GROUNDED" : "LLM_ONLY" }),
                reasoning:  `Destination overview for "${name}" generated by LLM` +
                            (imageUrl ? " with destination image from image API." : " (no image resolved — LLM only)."),
                sources:    ["LLM knowledge base", ...(imageUrl ? ["Destination image API"] : [])],
            }) });
        } catch (err) {
